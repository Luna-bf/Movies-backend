...
import { siblingCreation } from '../middlewares/siblingValidation.js'
...
siblingRouter.post('/sibling', async (req, res) => {
    ...
    try{
        ...
        newSibling.save()
        return res.status(201).json({ message: 'A new sibling has been created' })
    }
    catch (err) {
        console.log(err)
        return res.status(500).json({ message: 'Internal server error' })
...
siblingRouter.put('/sibling/:id', async (req, res) => {
    const {id} = req.params
    try{
       const updatedSibling = await User.findByIdAndUpdate(id, req.body, {new : true})
...
siblingRouter.delete('/sibling/:id', async (req, res) => {
    ...
    try{
        ...
        if(deleteSibling.deletedCount === 1){
            return res.status(203).json({message : 'Sibling has been deleted'})
        }
        return res.status(400).json({message : 'Sibling not found'})
     }
     catch (err) {
